...
# -*- coding: utf-8 -*-
"""
Created on Fri Oct 29 12:39:44 2021
...
#%% 3D
    ...
    def __init__(self,mypath,weightoutputpath,chkpointfname,NumberofTrainingSamples,epochs,batch_size,imgshape,patch_size,depth_size,totalbatchiterations,saveweightflag):
         ...
         self.checkpoint_prefix = chkpointfname
...
chkpointfname='/home/arun/Documents/PyWSPrecision/Pyoutputs/eddie_results/alpha/run1/checkpoints/0.index'

cGAN=CycleGAN(mypath,weightoutputpath,chkpointfname,NumberofTrainingSamples=1000,epochs=2,
              batch_size=1,imgshape=(512,512,1),patch_size=16,depth_size=32,totalbatchiterations=1,
              saveweightflag=False)

cGAN.load_checkpoint()

CT,CBCT=cGAN.dataload3D_2_predict()
CTsiz1=CT.shape 
CBsiz=CBCT.shape
...
for i in  range(0,CTsiz1[0],cGAN.patch_size*2):
    for j in range(0,CTsiz1[1],cGAN.patch_size*2):
        for zi in range(0,CTsiz1[2],cGAN.depth_size):
            currentBlk=CT[i:i+cGAN.patch_size*2,j:j+cGAN.patch_size*2,zi:zi+cGAN.depth_size]
            ...
            if blksiz[2] != cGAN.depth_size:
                ...
                # print('i=%s j=%s zi1=%')
            currentBlk_i=np.expand_dims(currentBlk, axis=0)
            currentBlk_t=tf.convert_to_tensor(currentBlk_i, dtype=tf.float32)
            currentBlk_p = cGAN.G(currentBlk_t, training=False)[0].numpy()
            disc_real_x = cGAN.X(currentBlk_i, training=False)
            disc_fake_x = cGAN.X(currentBlk_p, training=False)
            Disc1loss.append(disc_real_x)
            Disc2loss.append(disc_fake_x)
            currentBlk_p = np.squeeze(currentBlk_p,axis=3)
            CT_P[i:i+cGAN.patch_size*2,j:j+cGAN.patch_size*2,zi:zi+cGAN.depth_size]=currentBlk_p
...
for di in range(Dlen):
    D1_loss.append(tf.reduce_mean(Disc1loss[di]).numpy())
    D2_loss.append(tf.reduce_mean(Disc2loss[di]).numpy())
#%%
# error=CT-CT_P
# G_loss=np.mean(error)
...
#         print(epoch)
import matplotlib.pyplot as plt
plt.figure(1),plt.subplot(121),plt.imshow(CT[:,:,2],cmap='gray'),plt.show(),plt.subplot(122),plt.imshow(CT_P[:,:,2],cmap='gray'),plt.show()