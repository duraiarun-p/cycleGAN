...
# Define the loss function for the generator
    ...
    def set_learning_rate(self):
        self.G_optimizer.learning_rate.assign(self.current_learning_rate)
        self.F_optimizer.learning_rate.assign(self.current_learning_rate)
        self.X_optimizer.learning_rate.assign(self.current_learning_rate)
        self.Y_optimizer.learning_rate.assign(self.current_learning_rate)
...
# real_x, real_y = batch_data
 ...
 # Horse to fake zebra
    ...
    def pre_train_step(self, real_x, real_y):
        # self.mycompile_1()
# real_x, real_y = batch_data
 # Horse to fake zebra
        fake_y = self.G(real_x, training=True)
    ...
    def cycle_apply_grad(self,total_loss_G,total_loss_F,disc_X_loss,disc_Y_loss):
        with tf.GradientTape(persistent=True) as tape:
...
# real_x, real_y = batch_data
 ...
 # Horse to fake zebra
    ...
    def train_network_on_batch_per_epoch(self):
        ...
        with tf.GradientTape(persistent=True) as tape:
            for epoch in range(self.epochs):
                ...
                for batch_index in range(self.totalbatchiterations):
                    real_x,real_y=next(iter(train_ds))
 ...
 # Horse to fake zebra
                ...
                # Total generator loss
                total_loss_G_ele = gen_G_loss + cycle_loss_G + id_loss_G
                total_loss_F_ele = gen_F_loss + cycle_loss_F + id_loss_F
                
                # Discriminator loss
                disc_X_loss_ele = self.discriminator_loss_fn(disc_real_x, disc_fake_x)
                disc_Y_loss_ele = self.discriminator_loss_fn(disc_real_y, disc_fake_y)
                
                total_loss_G_lis.append(total_loss_G_ele)
                total_loss_F_lis.append(total_loss_F_ele)
                disc_X_loss_lis.append(disc_X_loss_ele)
                disc_Y_loss_lis.append(disc_Y_loss_ele)
                
                
                
            total_loss_G=tf.reduce_mean(total_loss_G_lis)
            total_loss_F=tf.reduce_mean(total_loss_F_lis)
            disc_X_loss=tf.reduce_mean(disc_X_loss_ele)
            disc_Y_loss=tf.reduce_mean(disc_Y_loss_ele)
    ...
    def learning_rate_finder(self):
        ...
        for epoch in range(self.epochs):
            ...
            for batch_index in range(self.totalbatchiterations):
                ...
                train_results = self.train_step(real_x=x, real_y=y)
            #     loss_G, loss_F, disc_X_loss_ele, disc_Y_loss_ele = self.pre_train_step(real_x=x, real_y=y)
                
            #     total_loss_G_lis.append(loss_G)
            #     total_loss_F_lis.append(loss_F)
            #     disc_X_loss_lis.append(disc_X_loss_ele)
            #     disc_Y_loss_lis.append(disc_Y_loss_ele)
            ...
            # disc_Y_loss=tf.reduce_mean(disc_Y_loss_lis)
            
            # self.cycle_apply_grad(total_loss_G,total_loss_F,disc_X_loss,disc_Y_loss)
                
            test_x,test_y=next(iter(test_ds))    
            test_results = self.test_step(real_x=test_x, real_y=test_y)
            
            # train_results=[total_loss_G.numpy(), total_loss_F.numpy(),disc_X_loss.numpy(), disc_Y_loss.numpy()]
            
            trainresult.append(train_results)
            testresult.append(test_results)
            # self.save_checkpoint(str(epoch))
            print('Epoch: %s Learning rate: %s'%(epoch,self.current_learning_rate))
        
            
        return trainresult,testresult
    
    def learning_rate_finder_1(self):
        ...
        for epoch in range(self.epochs):
            self.current_learning_rate=learning_rates[epoch]
            self.set_learning_rate()
            train_results=self.train_network_on_batch_per_epoch()
            test_x,test_y=next(iter(test_ds))    
            test_results = self.test_step(real_x=test_x, real_y=test_y)
...
cGAN=CycleGAN(mypath,weightoutputpath,NumberofTrainingSamples=1000,epochs=10,batch_size=1,imgshape=(512,512,1),patch_size=16,depth_size=32,totalbatchiterations=10,saveweightflag=False)
# cGAN.load_checkpoint()
# train_res,test_res=cGAN.train_network()
cGAN.mycompile_1()
lrs=cGAN.learningrate_log_scheduler()
train_res,test_res=cGAN.learning_rate_finder_1()
