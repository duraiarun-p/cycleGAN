...
def custom_loss_3_gamma(y_true, y_pred):# MS-SSIM
    ...
    for batchelei in range(batchsize[0]):
        y_pred1=y_pred[batchelei,:,:,:]
        y_true1=y_true[batchelei,:,:,:]
        ssimscoreele=ssTF.tfmssim_custom(y_true1, y_pred1, max_val,_MSSSIM_WEIGHTS,filter_size,filter_sigma)
        ssimscores.append(ssimscoreele)
    ssimscore=tf.reduce_mean(ssimscores)
...
class CycleGAN():     
    ...
    def __init__(self,mypath,weightoutputpath,epochs,batch_size,imgshape,totalbatchiterations,saveweightflag):
        ...
        # Combined model trains generators to fool discriminators
         ...
         self.cycleGAN_Model.compile(loss=['mse', 'mse', 'mae', 'mae','mae', 'mae', custom_loss_6_zeta,custom_loss_6_zeta],
    ...
    def traincgan(self):
        ...
        G_losses = np.zeros((self.totalbatchiterations,9,self.epochs))
        for epochi in range(self.epochs):
            for batchi in range(self.totalbatchiterations):
                ...
                g_loss = self.cycleGAN_Model.train_on_batch([batch_CT, batch_CB], [valid, valid, batch_CT, batch_CB, batch_CT, batch_CB, batch_CT, batch_CB])
...
# weightoutputpath='/home/arun/Documents/PyWSPrecision/Pyoutputs/cycleganweights/2192021/run2'
...
# batch_size=1
# epochs=1
cGAN=CycleGAN(mypath,weightoutputpath,epochs=1,batch_size=5,imgshape=(512,512,1),totalbatchiterations=2,saveweightflag=False)
...
# batch_CB = np.transpose(batch_CB,(2,0,1))
# batch_CB = np.expand_dims(batch_CB, -1)
# score=custom_loss_6_zeta(batch_CT,batch_CB)

batch_CT, batch_CB, G_losses, D_losses=cGAN.traincgan()
#%%
# TestGenCT2CB=cGAN.build_generator()
# TestGenCT2CB.load_weights("/home/arun/Documents/PyWSPrecision/Pyoutputs/cycleganweights/2992021/epsilon/run1/weights/GenCT2CBWeights-0.h5")
