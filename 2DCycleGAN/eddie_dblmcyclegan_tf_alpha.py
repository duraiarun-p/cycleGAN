...
class CycleGAN():     
    ...
    def dataload(self):
        ...
        yield batch_CT_img, batch_CB_img
    
    def get_ds(self):
        return tf.data.Dataset.from_generator(
            self.dataload,
            output_signature=(tf.TensorSpec(shape=(None, 256, 256, 1), dtype=tf.float32), tf.TensorSpec(shape=(None,256, 256, 1), dtype=tf.float32))
            )
...
# batch_size=1
# epochs=1
cGAN=CycleGAN(mypath,weightoutputpath,epochs=1,batch_size=3,imgshape=(256,256,1),totalbatchiterations=1,saveweightflag=True)
